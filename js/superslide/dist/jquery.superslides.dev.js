...
(function (window, $) {
  ...
  Superslides = function Superslides(el, options) {
    this.options = $.extend({
      play: false,
      animation_speed: 600,
    ...
    var that = this,
      ...
      "class": 'slides-control'
        ...
        multiplier = 1;
    this.$el = $(el);
    this.$container = this.$el.find(this.options.elements.container); // Private Methods
      ...
      children: function children() {
        ...
        if ($children.is('img')) {
          $children.each(function () {
            if ($(this).is('img')) {
              $(this).wrap('<div>'); // move id attribute
    ...
    var fx = {
      slide: function slide(orientation, complete) {
        var $children = that.$container.children(),
      ...
      fade: function fade(orientation, complete) {
        var that = this,
            $children = that.$container.children(),
    ...
    var image = {
      _centerY: function _centerY(image) {
        var $img = $(image);
        $img.css({
          top: (that.height - $img.height()) / 2
        });
      },
      _centerX: function _centerX(image) {
        var $img = $(image);
        $img.css({
          left: (that.width - $img.width()) / 2
        });
      },
      _center: function _center(image) {
        that.image._centerX(image);

        that.image._centerY(image);
      },
      _aspectRatio: function _aspectRatio(image) {
      ...
      _scale: function _scale(image, image_aspect_ratio) {
  ...
  Superslides.prototype = {
    ...
    _upcomingSlide: function _upcomingSlide(direction, from_hash_change) {
    ...
    _findSlideById: function _findSlideById(id) {
      return this.$container.find('#' + id).index();
    },
    _findPositions: function _findPositions(current, thisRef) {
    ...
    _parseHash: function _parseHash(hash) {
    ...
    animate: function animate(direction, userCallback) {
      var that = this,
          orientation = {};
